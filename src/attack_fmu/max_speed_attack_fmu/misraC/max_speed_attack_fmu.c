#include "max_speed_attack_fmu.h"
...
State* tick(State* st)
{
	...
	if (st->mode == X1 && ((st->time + 0.00001f) < st->attack_time)) {
		...
		enter(X1, st);
	} else if (st->mode == X1 && ((st->time + 0.00001f) > st->attack_time) && ((st->attack_duration - 0.00001f) <= 0.0f || ((st->time - st->attack_time + 0.00001f) < st->attack_duration))) {
#ifdef DBG
