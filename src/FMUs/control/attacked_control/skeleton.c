...
void initialize(const char* location) {
    init(&st);
    fmiBuffer.realBuffer[7] = st.servoLeftVal;
    fmiBuffer.realBuffer[8] = st.servoRightVal;
    fmiBuffer.realBuffer[10] = st.time;
...
if(first == 0){
    ...
    st.step_size = fmiBuffer.realBuffer[9] ;
    ...
    fmiBuffer.realBuffer[9] = st.step_size;*/
}
