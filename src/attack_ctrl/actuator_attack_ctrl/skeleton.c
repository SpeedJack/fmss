...
void doStep(ModelInstance* comp, const char* action)
{
	if(comp->first == 0) {
		...
		comp->st.attack_time = comp->fmiBuffer.realBuffer[11];
		comp->st.attack_duration = comp->fmiBuffer.realBuffer[12];
		comp->st.step_size = comp->fmiBuffer.realBuffer[14];
		comp->first = 1;
	}
	...
	comp->fmiBuffer.realBuffer[13] = comp->st.time;
	comp->fmiBuffer.realBuffer[9] = comp->st.servoLeftVal;
	comp->fmiBuffer.realBuffer[10] = comp->st.servoRightVal;
